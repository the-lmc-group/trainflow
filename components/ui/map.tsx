    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button
          ...
          title="Select layers"
          className={cn("absolute z-[1000] border", className)}
...
function MapMarker({
  ...
  }, [L, icon, iconAnchor, bgPos, popupAnchor, tooltipAnchor]);

  if (!L) return null;

  return <LeafletMarker icon={leafletIcon} riseOnHover {...props} />;
}
...
function MapZoomControl({ className, ...props }: React.ComponentProps<"div">) {
  ...
  return (
    <ButtonGroup
      ...
      className={cn("absolute z-[1000] h-fit shadow-md", className)}
